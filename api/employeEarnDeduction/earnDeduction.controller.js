const { create, update, checkInsertVal, checkUpdateVal, deleteByID, getDataById,
    ...
    newRecommended, getTotalGrosssalaryById, createNewWageLog, deleteEarntype } = require('../employeEarnDeduction/earnDeduction.service');
...
module.exports = {
    ...
    createWage: (req, res) => {
        ...
        createNewWageLog(values, (err, results) => {
            if (err) {
                return res.status(200).json({
                    success: 0,
                    message: err
    ...
    deleteEarntype: (req, res) => {
        const id = req.params.id;
        deleteEarntype(id, (err, results) => {
            if (err) {
                logger.errorLogger(err)
                return res.status(400).json({
                    success: 0,
