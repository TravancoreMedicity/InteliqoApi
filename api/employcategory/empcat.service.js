...
module.exports = {
    ...
    getContractEndDetl: (callBack) => {
        pool.query(
            ...
            LEFT JOIN hrm_emp_category ON hrm_emp_category.category_slno=hrm_emp_master.em_category
            where em_cont_close is null and em_cont_renew is null and contract_renew_appr=0 and em_cont_end<=LAST_DAY(CURDATE()) 
    ...
    renewCategory: (callBack) => {
        pool.query(
            `SELECT * FROM hrm_emp_category where ecat_cont=1 and ecat_status=1`,
            [],
            (error, results, feilds) => {
                if (error) {
                    return callBack(error);
