...
module.exports = {
    ...
    getFirstlevelVerification: (callBack) => {
        pool.query(
            ...
            em_no,em_name,hrm_branch.branch_name, hrm_department.dept_name,hrm_dept_section.sect_name,em_doj, em_status
    ...
    getSecondlevelVerification: (callBack) => {
        pool.query(
            ...
            (case when verification_status='0' then "First level pending" when verification_status='2' then "Not Verified" else "Verified" end ) as verify_remark, em_status
             from hrm_emp_verification
            left join hrm_emp_master on hrm_emp_master.em_id=hrm_emp_verification.em_id
            left join  hrm_branch ON hrm_branch.branch_slno = hrm_emp_master.em_branch
            left join hrm_department ON hrm_department.dept_id = hrm_emp_master.em_department
